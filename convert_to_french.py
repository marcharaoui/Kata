...
# convert_to_french script: take an int and translates it to a french in string
...
class Int2French:
    ...
    def translate2french(self, number, noCardinal=True):
        if number in self.units:
            ...
            if remainder == 0:
                ...
                return 'cent-' + self.translate2french(number % 100) if first_value == 1 else self.units[first_value] + '-cent-' + self.translate2french(number % 100)
            ...
            if factor == 1:
                ...
                prefix = self.translate2french(factor, noCardinal=False) + '-mille'
...
if __name__ == "__main__":
    ...
    for case in test_cases:
        output = converter.translate2french(case)
        print(f"{case}: {output}")
